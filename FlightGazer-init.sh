...
if [ -n "$VFLAG" ]; then
	echo "  >> Main python script present: [ OK ]"
fi
...
if [ -n "$VFLAG" ]; then
	echo -e "  >> Detected Python: ${PYTHON_VERSION} [ OK ]"
fi

if [ -n "$VFLAG" ]; then
	echo "  >> Running rgbmatrix detection script..."
fi
if [ ! -f "${BASEDIR}/utilities/rgbmatrix_check.py" ]; then
	...
	if [ -f "${VENVPATH}/bin/python3" ]; then
		"${VENVPATH}/bin/python3" "${BASEDIR}/utilities/rgbmatrix_check.py" >/dev/null 2>&1
		RGBMATRIX_PRESENT=$?
	else
...
if [ -n "$VFLAG" ]; then
	echo -e "  >> rgbmatrix-check return code: ${RGBMATRIX_PRESENT}"
fi

if [ -n "$VFLAG" ]; then
	echo "  >> Checking if web-app is installed..."
fi
...
if [ ! -f "$CHECK_FILE" ] || [ "$CFLAG" = true ]; then
	...
	elif [ $(wc -l < "${BASEDIR}/emulator_config.json") -ne 32 ]; then # count the lines in the heredoc in this script
...
if [ -f "$LOGFILE" ]; then
	...
	if [ $LOGLENGTH -gt 4000 ]; then
		time_now=$(date '+%Y-%m-%d %H:%M')
		echo "$(tail -n 4000 $LOGFILE)" > "$LOGFILE"
		sed -i -e "1i********** $time_now --- This logfile has been truncated to the latest 4000 lines. (was $LOGLENGTH) **********\\" "$LOGFILE"
		echo "> Logfile housekeeping: truncated logfile to latest 4000 lines. (was $LOGLENGTH)"
	fi
fi
