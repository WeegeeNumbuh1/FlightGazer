...
VERSION: str = 'v.11.2.2 --- 2026-05-24'
import os
import argparse
import sys
import math
...
try:
    ...
    main_logger.debug("External modules load-in successful.")
...
try:
    ...
    main_logger.debug("Internal modules load-in successful.")
except Exception as e:
    main_logger.exception(f"{e}")
...
API_CACHE_DATABASE = Path(CURRENT_DIR, "API_cache.db")
...
if not INTERACTIVE and sys.__stdin__.isatty():
    FORGOT_TO_SET_INTERACTIVE = True

# if VERBOSE_MODE:
#     connected_loggers = []
#     for key in logging.Logger.manager.loggerDict:
#         connected_loggers.append(key)
#     main_logger.debug(f"Connected loggers: {", ".join(connected_loggers)}")
...
"""
API_cache_present: bool = False
""" True if the API persistent cache is available and present. """
api_db_performance: list = [0, 0, 0, 0., 0., 0, 0]
""" Info about how the API cache is performing.
[Total queries, queries with no result, general failures,
average response time (ms), last response time (ms), total commits,
queries with returned data] """
#--- running stats
...
def match_commandline(command_search: str, process_name: str) -> list:
    ...
    main_logger.debug(f"Checked {iter_} processes in {end} ms. Found {len(list_of_processes)} matching process(es).")
...
def probe1090() -> tuple[str | None, str | None]:
    ...
    if PREFER_LOCAL and is_posix:
        ...
        for _, json_1090 in enumerate(file_locations):
...
def read_1090_config() -> None:
    ...
    global rlat, rlon, dump1090_receiver_version, is_readsb, LOCATION_IS_SET
...
def probe_API() -> tuple[int | None, float | None]:
    ...
    try:
        ...
        main_logger.warning("API returned a response that cannot be parsed.")
        return None, None
    except Exception as e:
...
def configuration_check() -> None:
    ...
    def switchtime_calc(num: float) -> tuple[int]:
        """ Given an input site radius, outputs a tuple
        of switch times for 2, 3, 4+ planes, in seconds. """
        minimum = 15
        maximum = 30
        mid_low_limit = 20
        low_cutoff = 2
        high_cutoff = 20
        # basic linear stuff
        short_time_slope = (maximum - minimum) / (high_cutoff - low_cutoff)
        short_time_b = maximum - (short_time_slope * high_cutoff)
        mid_time_slope = (maximum - mid_low_limit) / (high_cutoff - low_cutoff)
        mid_time_b = maximum - (mid_time_slope * high_cutoff)
        if num <= low_cutoff:
            return maximum, mid_low_limit, minimum
        elif num >= high_cutoff:
            return maximum, maximum, maximum
        else:
            short_time = int(short_time_slope * num + short_time_b)
            mid_time = int(mid_time_slope * num + mid_time_b)
            if short_time < minimum:
                short_time = minimum
            elif short_time > maximum:
                short_time = maximum
            if mid_time < minimum:
                mid_time = minimum
            elif mid_time > maximum:
                mid_time = maximum
            return maximum, mid_time, short_time
    ...
    if not NODISPLAY_MODE:
        ...
        if (
            ...
            or not isinstance(SCROLLING_SPEED, int)
            or not (10 <= SCROLLING_SPEED <= 50)
        ):
            main_logger.warning("SCROLLING_SPEED is out of bounds or not an integer.")
            main_logger.info(f">>> Setting to default ({default_settings['SCROLLING_SPEED']})")
            SCROLLING_SPEED = default_settings['SCROLLING_SPEED']
        else:
            if (
                JOURNEY_PLUS
                or ENHANCED_READOUT
                or ENHANCED_READOUT_AS_FALLBACK
            ) and SHOW_EVEN_MORE_INFO:
                if VERBOSE_MODE or SCROLLING_SPEED != default_settings['SCROLLING_SPEED']:
                    main_logger.info(f"Scrolling speed is set to {SCROLLING_SPEED}")
    ...
    if not NOFILTER_MODE:
        ...
        if not isinstance(LOCATION_TIMEOUT, int) or (15 < LOCATION_TIMEOUT > 60):
            main_logger.warning("LOCATION TIMEOUT is out of bounds or not an integer.")
        ...
        normal, mid, fast = switchtime_calc(RANGE * distance_multiplier)
        plane_latch_times = [
            int(normal // LOOP_INTERVAL),
            int(mid // LOOP_INTERVAL),
            int(fast // LOOP_INTERVAL)
        ]
        main_logger.debug(f"Plane latch times (loops): {plane_latch_times}")
            ...
            if API_COST_LIMIT is not None:
                ...
                if api_cost < API_COST_LIMIT:
                    ...
                    if API_cost_limit_reached:
                        main_logger.info("There are credits available again, API will be re-enabled.")
...
class PrintToConsole:
    ...
    def print_to_console(self, message) -> None:
        ...
        reference_time = time.monotonic()
        run_time = reference_time - START_TIME
        ...
        for aircraft in relevant_planes:
            try:
                ...
                if aircraft['NavigationAccuracy'] and aircraft['NavigationAccuracy'] <= 3:
                    print_info.append("?")
            ...
            except Exception: # gracefully handle where it breaks
        ...
        if result:
            ...
            if api_aka and api_flight != api_aka:
                api_str.append(f" ({api_aka})")
            api_str.append(": ")
        ...
        if WX_API_data:
            ...
            if (winds_ := WX_API_data['wind_speed']) > 0:
                wx_str.append(f"{winds_:.1f}{WX_API_data['wind_speed_unit']} "
                              f"@ {WX_API_data['wind_dir']}° | ")
            else:
                wx_str.append("Calm | ")
            wx_str.append("vis ")
            if (vis_ := WX_API_data['visibility']):
                wx_str.append(f"{vis_:.2f}{WX_API_data['distance_unit']}")
            else:
                wx_str.append("N/A")
            wx_str.append(", ceil ")
        ...
        # verbose stats line 5 (API results cache)
        if VERBOSE_MODE and API_cache_present:
            print(
                "> API cache stats: "
                f"Total queries: {api_db_performance[0]}, "
                f"{api_db_performance[6]}/{api_db_performance[1]} hits/misses,"
                f" errors: {api_db_performance[2]}, commits: {api_db_performance[5]} | Retrieval times:"
                f" {api_db_performance[3]:.3f} ms avg, {api_db_performance[4]:.3f} ms last "
            )
        ...
        for entry in reversed(unique_planes_seen):
            ...
            if entry['ID'] == input_ID:
                if (time_now - entry['Time']) < stale_age:
                    ...
                    add_entry()
                    return
        # finally, if we don't find the entry, add a new one
        add_entry()
    ...
    def plane_selector(self, message):
        ...
        if not NOFILTER_MODE:
            if plane_count > 0:
                ...
                # do some analysis on how the algorithm is handling the traffic in the area
                # these cutoff values/criteria may be adjusted in future updates, who knows
                ...
                warn_reason = ''
                if self.plane_count_avg >= 8 and iter_time > 30:
                ...
                if focus_plane_iter > 0:# clean-up variables
                    ...
                    if focus_plane_iter * LOOP_INTERVAL < 1800:
                        ...
                        if len(self._active_loop_count) == 0:
...
class APIFetcher:
    ...
    def get_API_results(self, message):
        ...
        with any attempt to query the API, failures included. If the persistent API cache is available, queries will look
        at the cache first, and if not, will use the actual API for a result, then append the result to the cache. """
        ...
        for result in reversed(focus_plane_api_results):
            try:
                access_time_delta = reference_time - result['APIAccessed']
                last_status = result['Status']
                if result is not None and focus_plane == result['ID']: # cache hit...
                    if access_time_delta < stale_age and last_status <= 1:
                        api_hits[3] += 1
                        return # no need to query the API, API result is still valid
                    elif access_time_delta < 300 and last_status >= 2:
                        api_hits[3] += 1
                        return # only try to poll the API again after 5 minutes
        ...
        if API_cache_present:
            ...
            if cache_result:
                origin = cache_result.get('Origin')
                identity = cache_result.get('Identity')
                origin_icao = cache_result.get('OriginICAO')
                destination = cache_result.get('Destination')
                destination_icao = cache_result.get('DestinationICAO')
                origin_city = cache_result.get('OriginCity')
                origin_name = cache_result.get('OriginName')
                destination_city = cache_result.get('DestinationCity')
                destination_name = cache_result.get('DestinationName')
                API_status = 4
                flight_type = "Airline"
            api_db_performance[0] = api_cache.queries
            api_db_performance[1] = api_cache.query_misses
...
class DisplayFeeder:
    ...
    def data_packet(self, message):
        ...
        if general_stats: # should always exist but just in case
            if LOCATION_IS_SET or NOFILTER_MODE:
                p_count = len(unique_planes_seen)
                if p_count <= 9999:
                    total_flybys = f"{p_count}"
                elif p_count > 9999:
                    total_flybys = "9999"
                else:
                    total_flybys = "0"
        ...
        if focus_plane and focus_plane_stats:
            ...
            if len(flight_name) > 8:
                if flight_name.endswith("*"):
                    flight_name = flight_name[:7] + "*."
                else:
                    flight_name = flight_name[:8] + "."
            ...
            if not aircraft_str_ and focus_plane_stats['ID'].startswith("~"):
                aircraft_str_ = ["NO ADD'L INFO (TIS-B Contact)"]

            # Generate more detailed journey detail/provide API error messages
            # Examples:
            ...
            if not api_limiter_reached():
                ...
                if API_status == 0 or API_status == 4:
                    ...
                    journey_str = "No API result for this flight."
                elif API_status == 2:
                    journey_str = "API call for this flight failed."
                elif API_status == 3:
                    journey_str = "Connection error, check network."
            if not ENHANCED_READOUT_AS_FALLBACK: # only show the below if ENHANCED_READOUT isn't used
                if api_limiter_reached():
                    if API_cost_limit_reached:
                        journey_str = "API cost limit reached."
                    elif API_schedule_triggered:
                        journey_str = "No API calls this hour."
                    elif API_daily_limit_reached:
                        journey_str = "API daily call limit reached."
                elif not API_KEY:
                    journey_str = "API disabled, route info unavailable."
...
class WriteState:
    ...
    def export_FlightGazer_state(self, message) -> None:
        ...
        try:
            ...
            api_db_stats: dict | None = None
            if API_cache_present:
                api_db_stats = {
                    'total_queries': api_db_performance[0],
                    'hits': api_db_performance[6],
...
def API_Scheduler() -> None:
    ...
    if not isinstance(API_SCHEDULE, dict) or not API_SCHEDULE or not API_SCHEDULE.get('ENABLED', False):
        ...
        return
    main_logger.info("API_SCHEDULE is enabled. Checking its validity...")
...
class DistantDeterminator():
    ...
    def debug_switch(self, message):
        if self._detected:
            main_logger.debug("No longer detecting extremely distant aircraft.")
        self._detected = False
...
class Display(
    ...
    def aa_scene_switch(self, count):
        ...
        if (self.active_plane_display
            ...
            self._last_framerate = self.framerate
            self.framerate = SCROLLING_SPEED
    ...
    def j_idle_stats_2(self, count):
        ...
        if CLOCK_CENTER_ROW_2ROWS and self._last_row2_data != row2_data:
...
if DISPLAY_IS_VALID and not NODISPLAY_MODE:
    ...
    if not EMULATE_DISPLAY and is_posix and os.geteuid() != 0:
    ...
    try:
        ...
        DISPLAY_IS_VALID = False
        main_logger.exception("RGBMatrixEmulator failed to start. No display will be available!")
        time.sleep(5)
    except Exception as e:
        DISPLAY_IS_VALID = False
